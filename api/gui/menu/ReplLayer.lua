...
function ReplLayer:on_query()
   ...
   if self.scrollback:len() == 0 then
      if self.message then
         ...
         self:print(string.format("%s ver. %s\nLÖVE version: %s  Lua version: %s  OS: %s",
                                  Env.program_name(), Env.version(), Env.love_version(), Env.lua_version(), Env.os()))
      end
   end
end
...
function ReplLayer:relayout(x, y, width, height)
   ...
   self.t = {
      repl_bg_color = {17, 65, 65, 192},
      repl_error_color = {255, 0, 0},
      repl_result_color = {150, 200, 200},
      repl_completion_color = {255, 240, 130},
      repl_search_color = {130, 240, 130},
      repl_match_color = {17, 17, 200}
   }
...
function ReplLayer.format_repl_result(result)
   ...
   if type(result) == "table" then
      ...
      if tostring(result) == "<generator>" then
